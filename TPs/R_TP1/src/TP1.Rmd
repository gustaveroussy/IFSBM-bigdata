...
into them and outputs their new representation to the following layer until the last layer that provides a representation
suitable for performing a given tasks.

**INCLASS WORK**  Define a neural network by defining a sequence of three dense layers (the input layer of size 128, one
intermediate layer of size 30, and the output layer of size 1). For this first exercise, you have to build a network
with only one hidden layer (with 30 neurons). The input layer should take as input a vector of size the number of genes
in the `TcgaData$EXP` table while the output layer should produce a size-1 vector of "class probability"
...
Train it on 50 epochs using a validation split of 20%. Save the output of the `fit` function in a variable called
`history`. 

*Hint*: You will need the `trainBlcaIndex` variable to select only rows in the training set and you will need to use the
"BINARY_TUMOR_STAGE_NUM" column in the `TcgaData$CLINIC` table for the labels. You can apply the function `plot` on the
`history` variable to draw the learning curves.
...
*Hint*: You will need to use `trainBlcaIndex` and `testBlcaIndex` variables to retrieve the labels predicted by the model
on the training and test sets. You will need to cast the dataframe `TcgaData$EXP` into a matrix after having selected
the rows using the `as.matrix` function.
...
**QUESTION 10)**  How does the test accuracy compare against the train accuracy? The validation accuracy? How does the
...
This part is OPTIONAL. It will contribute **bonus points** in case you decide to perform this part.
