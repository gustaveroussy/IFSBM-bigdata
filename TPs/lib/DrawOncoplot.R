...
oncoplot <- function(maf, top = 20, minMut = NULL, genes = NULL, altered = FALSE,
  ...
  if(drawRowBar){
    ...
    if(drawColBar){
      par(mar = c(0.25, 0, 2, pct_mar), xpd = TRUE)
  ...
  if(!is.null(clinicalFeatures)){
    # move to next column right away
    y_axp <- 1
    x_axp <- x_axp + max_w_cur
    max_w_cur <- 0
    n_y_cur <- 0
...
DrawOncoplot <- function(obj_maf, genes_list, df_left_bar_data, df_right_bar_data, oncoplot_colors, n_genes, n_tumors, 
                         draw_titv=T, showTumorSampleBarcodes=T, font_size=0.65, clinicalFeatures=NULL,
