...
```{r draw oncoplot, echo=F, fig.height=8, fig.width=8, fig.align="center", fig.cap="Oncoplot TCGA LIHC", dpi=300, eval=eval_chunks}
...
**INCLASS WORK**: Visualize the activities of the signatures in your set of patients. 

```{r visualize signature activities, eval=eval_chunks}
# YOUR WORK HERE
```
...
```{r load data survival, eval=eval_chunks}
...
```{r remove bad survival data, eval=eval_chunks}
...
```{r plot survival data, fig.height=6, fig.width=6, fig.align="center", eval=eval_chunks}
...
You should look for the value on the x-axis where the survival curves meets the probability 0.5 on the y-axis. Using
this technique, the median survival time of these patients was approximately 60 months, or 5 years.
...
# select among CgcGenes only the genes mutated in at least 3% of the cohort
...
```{r fit glmnet cox cv, eval=eval_chunks}
fitCoxCv <- cv.glmnet(
  x=#YOUR WORK HERE. Don't forget to cast as.matrix,
  y=#YOUR WORK HERE,
  family=#YOUR WORK HERE,
  alpha=1,
  nfolds=10,
  type.measure="C"
)
plot(fitCoxCv)
print(paste("You best value of lambda is :", fitCoxCv$lambda.min))
```

```{r coef glmnet cox best, fig.cap="Top Cox model coefficients", fig.height=6, fig.width=6, fig.align="center", eval=eval_chunks}
fitCoxReduced <-glmnet(
  x=# YOUR WORK HERE,
  y=# YOUR WORK HERE,
  family=# YOUR WORK HERE,
  alpha=1,
  lambda=# YOUR WORK HERE
)

fitCoxCoefs <- as.matrix(coefficients(fitCoxReduced))
...
The C-index measures, out of all pairs of patients that are comparable, the proportion of pairs that are correctly
ranked by the model. A pair of patient is incomparable only if the first patient was censored before the other patient
died as we don't know if the first patient survived longer or not. For all other cases, the pair is correctly ranked by
the model if it predicts a lower risk for the patient that lived longer. Our model has a very poor performance with a
C-index value in the range of 0.52-54 (0.5 is the C-index value for a **random** model). This model SHOULD NOT be used
in the clinic.
