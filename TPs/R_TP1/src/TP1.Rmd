...
getColors <- function(vec, pal="Dark2", alpha=0.7){
  ...
  palette_predefined <- read.csv("../data/colors.csv") # Ensure file exists or comment out
  # Fallback
...
drive_deauth() # Use public access
...
# (You can just list the filenames if they are the same on Drive and locally)
files_to_sync <- c(
...
# 3. Get the list of ALL files in the Drive folder once
# This is more efficient than searching for each file individually
message("Fetching folder contents from Google Drive...")
drive_contents <- drive_ls(path = as_id(data_url))

# 4. Iterate through your list and download if missing locally
for (filename in files_to_sync) {
  ...
  if (!file.exists(local_path)) {
    ...
    # Find the specific file entry in the drive contents we fetched earlier
    # We look for an exact name match
    ...
    if (nrow(target_entry) == 0) {
      ...
      drive_download(as_id(target_entry$id), path = local_path, overwrite = TRUE)
    }
...
colorsStudy <- getColors(TcgaDataBin$CLINIC$Project_TCGA, alpha=1)
